...
public class GameActivity extends AppCompatActivity {
        ...
        imgBoard.setOnTouchListener(new View.OnTouchListener() {
            ...
            public boolean onTouch(View v, MotionEvent event) {
                if(event.getAction()==MotionEvent.ACTION_DOWN){
                    if(!board.onTouch(v,event,STATE.PLAYER))return false;
                    ...
                    board.BestAiMove(board.prvRow,board.prvCol,v);
    ...
    void checkWinner(){
        if(board.isGameEnd==true){
            if(board.ST_ENDGAME== STATE.PLAYER)
                txtWinner.setText("PLAYER WIN");
            else if(board.ST_ENDGAME==STATE.COMPUTER)
                txtWinner.setText("COMPUTER WIN");
    ...
    private void initWidgets(){
        ...
        board=new Board(GameActivity.this,canvas,paint,BM_WIDTH,BM_HEIGHT,NUM_ROW,NUM_COL);
